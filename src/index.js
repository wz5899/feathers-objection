...
class Service extends AdapterService {
  constructor(options) {
    ...
    this.allowedUpsert =
      options.allowedUpsert && RelationExpression.create(options.allowedUpsert);
  ...
  getIdsQuery(id, idList, addTableName = true) {
    ...
    if (Array.isArray(this.id)) {
      ...
      query[addTableName ? `${this.Model.tableName}.${this.id}` : this.id] =
        idList ? (idList.length === 1 ? idList[0] : { $in: idList }) : id;
    }
  ...
  _selectQuery(q, $select) {
    if ($select && Array.isArray($select)) {
      const items = $select.concat(
        Array.isArray(this.id)
          ? this.id.map((el) => {
  ...
  _selectAliases($select) {
    ...
    return $select.reduce((result, item) => {
      ...
      if (matches) {
        ...
        const field = tableField.startsWith(`${this.Model.tableName}.`)
          ? tableField.substr(this.Model.tableName.length + 1)
          : tableField;
  ...
  _selectFields(params, originalData = {}) {
    return (newObject) => {
      ...
      if (
        params.query &&
        params.query.$select &&
        !params.query.$select.find(
          (field) => field === '*' || field === `${this.Model.tableName}.*`
        )
      ) {
  ...
  _checkUpsertId(id, newObject) {
    ...
    Object.keys(updateId).forEach((key) => {
      if (!Object.prototype.hasOwnProperty.call(newObject, key)) {
        ...
        throw new errors.BadRequest(
          `Id '${key}': values mismatch between data '${newObject[key]}' and request '${updateId[key]}'`
        );
      }
    });
  }
  ...
  createQuery(params = {}) {
    ...
    if (query && query.$mergeEager) {
      q[joinEager ? 'withGraphJoined' : 'withGraphFetched'](
        query.$mergeEager,
        eagerOptions
      );
    ...
    const find = (params, count, filters, query) => {
      ...
      if (count) {
        const countColumns =
          groupByColumns ||
          (Array.isArray(this.id)
            ? this.id.map((idKey) => `${this.Model.tableName}.${idKey}`)
            : [`${this.Model.tableName}.${this.id}`]);
        ...
        return countQuery
          .then((count) =>
            count && count.length ? parseInt(count[0].total, 10) : 0
          )
          .then(executeQuery)
          .catch(errorHandler);
      }
  ...
  _get(id, params = {}) {
    ...
    return this._find(Object.assign({}, params, { query: findQuery })).then(
      (page) => {
  ...
  async _create(data, params = {}) {
    ...
    return promise
      .then((insertResults) =>
        this._getCreatedRecords(insertResults, data, params)
      )
      .then(
          ...
          .then(async (meta) => {
            ...
            if (allowedUpsert) {
              ...
              return this._createQuery(params)
                ...
                .upsertGraphAndFetch(newObject, upsertGraphOptions)
                .then(
                  this._commitTransaction(transaction),
                  this._rollbackTransaction(transaction)
                );
  ...
  _patch(id, data, params = {}) {
    ...
    const mapIds = (page) =>
      Array.isArray(this.id)
        ? this.id.map((idKey) => [
            ...new Set((page.data || page).map((current) => current[idKey])),
          ])
